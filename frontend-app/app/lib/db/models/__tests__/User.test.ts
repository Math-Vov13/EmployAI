...
describe('User Model', () => {
  describe('passwordSchema', () => {
    ...
    test('should reject password shorter than 12 characters', () => {
      ...
      if (!result.success) {
        expect(result.error.issues[0].message).toContain('le MDP doit au moins contenir 12 caractères.');
      }
    });
    ...
    test('should reject password without uppercase letter', () => {
      ...
      expect(result.success).toBe(false);
      if (!result.success) {
        expect(result.error.issues[0].message).toContain('Le mot de passe doit avoir au moins une MAJUSCULE :)');
      }
    });
    ...
    test('should reject password without lowercase letter', () => {
      ...
      expect(result.success).toBe(false);
      if (!result.success) {
        expect(result.error.issues[0].message).toContain('Le mot de passe doit avoir au moins une minuscule... c:');
      }
    });
    ...
    test('should reject password without number', () => {
      ...
      expect(result.success).toBe(false);
      if (!result.success) {
        expect(result.error.issues[0].message).toContain('Le mot de passe doit avoir au moins un chiffre... 0-9');
      }
    });
    ...
    test('should reject password without special character', () => {
      ...
      expect(result.success).toBe(false);
      if (!result.success) {
        expect(result.error.issues[0].message).toContain('Le mot de passe doit contenir au moins un caractère spécial.');
      }
    });
  ...
  describe('toUserResponse', () => {
    ...
    test('should handle Google OAuth user', () => {
      ...
      expect(response.picture).toBe('https://lh3.googleusercontent.com/a/avatar');
